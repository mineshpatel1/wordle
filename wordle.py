...
class Game:
    ...
    @property
    def information(self) -> tuple[dict[int, str], set[tuple[str, int]], set[str], dict[str, int]]:
        return get_info_from_hints(self.guesses, self.hints)
    ...
    def information_value(self) -> float:
        return -1 * math.log(len(self.possible_answers) / len(self.word_list), 2)
...
def bot_play_random(
    ...
    while not game.is_over:
        possible_answers = game.possible_answers
        if filter_by_list:
...
def bot_play(
    ...
    while not game.is_over:
        guess = get_best_move(game.possible_answers, word_list, answer_list)
